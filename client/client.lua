...
local function getIGTemp()
	-- Get Temperatures
	local Faharenhiet = ShouldUseMetricTemperature();
	local temperature
...
local function nativeAlertTop() 
	...
	if zone then
		--exports['rsg-core']:Notify(9,''..time..''..temp..''..zone..'',Config.Notification.TimeShowing, 0, 'generic_textures', 'tick',"COLOR_YELLOW")
		RSGCore.Functions.Notify(Lang:t(9,''..time..''..temp..''..zone..'',Config.Notification.TimeShowing, 0))
	end
end
